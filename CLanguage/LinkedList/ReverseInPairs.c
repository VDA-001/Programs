...
void reverseInPair(NODE * hed){
    if(!hed || !hed->addr){
        return;
    }
    int temp;
    temp = hed->data;
    hed->data = hed->addr->data;
    hed->addr->data = temp;
    reverseInPair(hed->addr->addr);
}
...
int main(){
    ...
    for(;;){
        printf("Enter the operation to perform (1.Insert , 2.Display , 3.Delete ,4.Reverse In Pair, 5.Exit,): ");
        scanf("%d",&n);
        switch (n)
        {
        case 1:
